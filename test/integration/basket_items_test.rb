...
class BasketItemsTest < ActionDispatch::IntegrationTest
  ...
  test "basket item add remove" do
    ...
    assert_difference 'BasketItem.count', -1 do
      delete basket_item_path(BasketItem.where(promotion_id: @twenty_pct_off_promotion.id).first)
    end
    assert_difference 'BasketItem.count', -1 do
      delete basket_item_path(BasketItem.where(item_id: @motion_sensor_item.id).first)
    end
  end
  ...
  test "code promotions conjunctions" do
    ...
    delete basket_item_path(BasketItem.where(promotion_id: @twenty_eur_off_promotion.id).first)
    delete basket_item_path(BasketItem.where(promotion_id: @five_pct_off_promotion.id).first)
  ...
  test "calculate basket total amount" do
    ...
    post basket_items_path, params: { basket_item: { type: 'UsingPromotion',
                                                     basket_id: 1,
                                                     promotion_id: @twenty_eur_off_promotion.id } }
    assert_equal Basket.find(1).calculate_total_amount, 365.693
  end
end
